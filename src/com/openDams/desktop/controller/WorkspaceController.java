...
public class WorkspaceController implements Controller{
	...
	public ModelAndView handleRequest(HttpServletRequest arg0, HttpServletResponse arg1) throws Exception {
		ModelAndView mav = new ModelAndView("desktop/workspace2");
		...
		mav.addObject("archiveList", service.getListFromSQL(Archives.class,"SELECT * FROM archives  inner join archive_user_role on archives.id_archive=archive_user_role.ref_id_archive inner join archive_group on archives.ref_id_archive_group=archive_group.id_archive_group where archive_user_role.ref_id_user="+user.getId()+" order by archive_group.archive_group_order,archives.archive_order;"));
		return mav;
	}
