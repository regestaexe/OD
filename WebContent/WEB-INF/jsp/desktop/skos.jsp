...
function next_prew_children_page(id_record,action,tot_page,fromRelations){
	...
	openLoading();
	$(".record_children_"+id_record).find(".record_list").load("ajax/skosDesktop.html?idArchive=<%=request.getParameter("idArchive")%>&action=get_children&idRecord="+id_record+"&current_page="+current_page,function(){
		closeLoading();
		build_tree();
	});
}
function getChildren(id_record){	
    if($(".record_children_"+id_record).is(":visible")){
    	...
    	openLoading();
    	$(".record_children_"+id_record).find(".record_list:first").load("ajax/skosDesktop.html?idArchive=<%=request.getParameter("idArchive")%>&action=get_children&idRecord="+id_record,function(){
