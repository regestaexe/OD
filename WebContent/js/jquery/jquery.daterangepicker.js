 ...
 * scott@filamentgroup.com http://www.filamentgroup.com reference article:
 * http://www.filamentgroup.com/lab/update_date_range_picker_with_jquery_ui/
 * demo page: http://www.filamentgroup.com/examples/daterangepicker/
 * 
 * Copyright (c) 2008 Filament Group, Inc Dual licensed under the MIT
 * (filamentgroup.com/examples/mit-license.txt) and GPL
 * (filamentgroup.com/examples/gpl-license.txt) licenses.
 * 
 * Dependencies: jquery, jquery UI datepicker, date.js library (included at
 * bottom), jQuery UI CSS Framework Changelog: 10.23.2008 initial Version
 * 11.12.2008 changed dateFormat option to allow custom date formatting (credit:
 * http://alexgoldstone.com/) 01.04.09 updated markup to new jQuery UI CSS
 * Framework 01.19.2008 changed presets hash to support different text
 * --------------------------------------------------------------------
 */
jQuery.fn.daterangepicker = function(settings) {
	...
	var options = jQuery.extend({
		...
		// accept date.js string or a function which returns a date object
		presets : {
			specificDate : 'Una sola data',
			allDatesBefore : 'Tutte le date prima di...',
			allDatesAfter : 'Tutte le date dopo di...',
			dateRange : 'Range di date'
		},
		rangeStartTitle : 'Data iniziale',
		rangeEndTitle : 'Data finale',
		...
		dateFormat : 'yymmdd', // date formatting. Available formats:
		// http://docs.jquery.com/UI/Datepicker/%24.datepicker.formatDate
		closeOnSelect : true, // if a complete selection is made, close the
		// menu
		arrows : false,
		posX : rangeInput.offset().left, // x position
		posY : rangeInput.offset().top + rangeInput.outerHeight(), // y
		// position
		appendTo : 'body',
		onClose : function() {
		},
	...
	// object containing native UI datepicker API
	// options
	}, settings);
	...
	if (options.arrows && rangeInput.size() == 1) {
		...
		jQuery(this).addClass('ui-rangepicker-input ui-widget-content').wrap('<div class="ui-daterangepicker-arrows ui-widget ui-widget-header ui-helper-clearfix ui-corner-all"></div>').before(prevLink).before(nextLink).parent().find('a').click(function() {
			...
			var diff = Math.abs(new TimeSpan(dateA - dateB).getTotalMilliseconds()) + 86400000; // difference
			// plus
			// one
			// day
			...
			rpPickers.find('.range-start, .range-end ').each(function() {
				...
				jQuery(this).datepicker("setDate", thisDate.add({
					milliseconds : diff
...
/*
 * TimeSpan(milliseconds); TimeSpan(days, hours, minutes, seconds);
...
/*
 * TimePeriod(startDate, endDate); TimePeriod(years, months, days, hours,
